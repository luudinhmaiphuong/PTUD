    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="w-full lg:w-1/4 space-y-8">
          ...
          {/* Categories */}
          <div>
            <h3 className="font-bold text-gray-500 uppercase tracking-wider mb-4 text-sm border-l-4 border-pink-300 pl-3">Danh mục sản phẩm</h3>
            <ul className="space-y-0 border-t border-gray-100">
              {categories.map((cat) => (
                <li key={cat.id} className="border-b border-gray-100">
                  <button className="w-full py-3 flex justify-between items-center text-gray-600 hover:text-pink-500 text-sm">
                    <span>{cat.name}</span>
                    <div className="flex items-center gap-2">
                      <span className="text-gray-300 text-xs">({cat.count})</span>
                      <ChevronDown size={14} />
                    </div>
                  </button>
                </li>
          ...
          {/* Price Filter */}
          <div>
            <h3 className="font-bold text-gray-500 uppercase tracking-wider mb-4 text-sm border-l-4 border-pink-300 pl-3">Giá</h3>
            <input 
              type="range" 
              min="0" 
              max="200000" 
            ...
            <div className="flex justify-between items-center mt-4">
              <button className="bg-gray-600 text-white px-4 py-1 rounded-full text-xs font-bold uppercase hover:bg-gray-700">Lọc</button>
              <span className="text-sm text-gray-500">Giá: 18.000 ₫ — {priceRange.toLocaleString('vi-VN')} ₫</span>
            </div>
          </div>

          {/* Rating Filter */}
          <div>
             <h3 className="font-bold text-gray-500 uppercase tracking-wider mb-4 text-sm border-l-4 border-pink-300 pl-3">Đánh giá trung bình</h3>
             <div className="flex items-center gap-2 text-sm text-gray-500 cursor-pointer hover:text-pink-500">
        ...
        <div className="w-full lg:w-3/4">
          ...
          <div className="flex justify-between items-center mb-6">
             <div className="text-sm text-gray-500">Hiển thị tất cả {products.length} kết quả</div>
              ...
              <div key={product.id} className="group">
                <div className="relative overflow-hidden rounded-lg mb-3 border border-gray-100">
                  ...
                  <div className="absolute top-2 right-2 bg-white p-1 rounded-full opacity-0 group-hover:opacity-100 transition-opacity shadow-md">
                      <img src="https://picsum.photos/30/30?random=logo" alt="logo-mini" className="w-6 h-6 rounded-full"/>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-1">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <Star 
                        key={star} 
                        size={14} 
                        className={star <= product.rating ? "text-yellow-300" : "text-gray-200"} 
                  ...
                  <div className="flex items-center gap-2">
                    {product.originalPrice && (
                      <span className="text-gray-400 line-through text-sm">{product.originalPrice.toLocaleString('vi-VN')} ₫</span>
                    )}
                    <span className="text-black font-bold text-lg">{product.price.toLocaleString('vi-VN')} ₫</span>
